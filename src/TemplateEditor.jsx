...
function TemplateEditor(props) {
    ...
    <div className="App">
      {
        (templates != null && templates[currentTemplate] != null) && (
          <div className='text-editor'>
            <textarea className='text-area' value={baseText} onChange={handleTextChange}></textarea>
            <Box style={{maxHeight: '90vh'}} className="box">
              {createForm()}
              <div className='editor-buttons'>
                <Button className="editor-button" variant="contained" onClick={addTemplateVariable}>Adicionar Variavel</Button>
              </div>
            </Box>
          </div>
        )
  ...
  function addTemplateVariable() {
    templates[currentTemplate].baseText += ` {{${templates[currentTemplate].options.length + 1}}}`
    templates[currentTemplate].options.push(null)
    setBaseText(templates[currentTemplate].baseText)
    handleTextChange({target: {value: templates[currentTemplate].baseText}})
  }
  ...
  function handleTextChange(event) {
    ...
    if (variablesCount != templates[currentTemplate].options.length) {
      for (let i = templates[currentTemplate].options.length; i < variablesCount; i++) {
        templates[currentTemplate].options.push(null)
      }
    }
  }
      ...
      case null:
        ...
        return (
          <FormControl key={id} sx={{ m: 1, minWidth: 120 }}>
            <InputLabel id={`select-label-${id}`}>Tipo da variavel{id}</InputLabel>
            <Select
